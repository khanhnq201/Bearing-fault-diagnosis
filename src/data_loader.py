...
class CWRU_dataloader: 
    def __init__(self, config):
        ...
        self.overlapping_ratio = config['overlapping_ratio']
        ...
        self.min_rpm = 1720
        self.n_rev = config['n_rev_resampling']
        self.resampling_output_size = int(self.sample_length/self.fs * self.min_rpm/60) * self.n_rev
        self.default_rpm = 1750
        
        # Store RPM for each file
        self.rpm_dict = {}

        self.nperseg = config['spectrogram']['nperseg']
        self.noverlap = int(config['spectrogram']['ratio'] * self.nperseg)
    ...
    def import_files(self, file_keys, overlapping_ratio=0):
        all_samples = []
        all_labels = []
        all_file_ids = []  # Track which file each sample came from
        base_path_obj = Path(self.base_path)

        if overlapping_ratio > 1 or overlapping_ratio < 0: 
            overlapping_ratio = 0
        step = int(self.sample_length * (1 - overlapping_ratio))
        if step < 1:
            step = 1
    ...
    def import_data(self):
        self.train_samples, self.train_labels, self.train_file_ids = self.import_files(self.train_list,self.overlapping_ratio)
    ...
    def _angular_resampling(self, signal, rpm):
        ...
        theta_target = np.arange(0, self.resampling_output_size * theta_target_step, theta_target_step)
    ...
    def raw(self):
        self.preprocessed_train_samples = self.train_samples[:, np.newaxis, :]
        self.preprocessed_val_samples   = self.val_samples[:, np.newaxis, :]
        self.preprocessed_test_samples  = self.test_samples[:, np.newaxis, :]


        print(f"Raw signal shape - Train: {self.preprocessed_train_samples.shape}, Val: {self.preprocessed_val_samples.shape}, Test: {self.preprocessed_test_samples.shape}")
    ...
    def get_samples_for_heatmap(self, processing_type, data_list, num_samples_each = 3):
        common_axis = None 

        if processing_type == 'raw':
            common_axis = np.arange(self.sample_length) / self.fs
        elif processing_type == 'freq_spectrum':
            N = self.sample_length
            common_axis = np.fft.fftfreq(N, d=1/self.fs)[:N//2]
        elif processing_type == 'order_spectrum':
            order_axis_length = self.resampling_output_size
            common_axis = np.arange(order_axis_length // 2) * self.n_rev/self.resampling_output_size
        else:
            raise ValueError(f"Unknown processing_type: {processing_type}")
        ...
        for key in file_keys:        
            ...
            num_samples_in_file = 0
            file_samples = []
            for i in range(1, num_samples_each + 1):
                start = int(self.sample_length * i/(num_samples_each+1))
                sample = time_series[start : start + self.sample_length]
                file_samples.append(sample)
                all_file_ids.append(key)
                num_samples_in_file += 1
        ...
        if processing_type == 'raw':
            X_final = np.array(X)[:, np.newaxis, :]
        elif processing_type == 'freq_spectrum':
            X_processed = []
            for signal in X:
                envelope = self._envelope_extraction(signal)
                spectrum = self._spectrum(envelope)
                X_processed.append(spectrum)
            X_final = np.array(X_processed)[:, np.newaxis, :]
